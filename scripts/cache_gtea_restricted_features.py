...
def run(args: argparse.Namespace) -> None:
    ...
    for video_path in sorted(args.videos.glob("*.mp4")):
        ...
        if selected_indices.size == 0:
            ...
            continue
        frames, metadata = read_video_at_indices(video_path, selected_indices)
        crops = [crop_frame(frame, available[int(index)]) for frame, index in zip(frames, selected_indices)]
        ...
        features = extractor.extract(crops, batch_size=args.batch_size)
